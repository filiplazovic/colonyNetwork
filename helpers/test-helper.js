...
module.exports = {
  ...
  async createSignatures(signers, multisigAddr, nonce, destinationAddr, value, data) {
    ...
    const input = `${'0x19'}${'00'}${multisigAddr.slice(2)}${destinationAddr.slice(2)}${web3Utils.padLeft(value.toString('16'), '64', '0')}${data.slice(2)}${web3Utils.padLeft(nonce.toString('16'), '64', '0')}`; // eslint-disable-line max-len

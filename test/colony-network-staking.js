...
contract('ColonyNetwork', (accounts) => {
  ...
  describe.only('when initialised', () => {
    ...
    it('should allow a user to back the same hash more than once in a same cycle with different entries, and be rewarded', async () => {
      ...
      // Check that they have had their staked balance increase
      const balance1Updated = await colonyNetwork.getStakedBalance(MAIN_ACCOUNT);
      assert.equal(balance1Updated.toString(), new BigNumber(3).times(new BigNumber(10).pow(18)).toString(), 'Account was not rewarded properly');
    ...
    it('should cope with many hashes being submitted and eliminated before a winner is assigned', async () => {
      ...
      for (let i = 0; i < accounts.length; i += 1) {
        await giveUserCLNYTokens(accounts[i], new BigNumber('1000000000000000000')); // eslint-disable-line no-await-in-loop
      ...
      await Promise.all(accounts.map(addr => clny.approve(colonyNetwork.address, new BigNumber('1000000000000000000'), { from: addr })));
      await Promise.all(accounts.map(addr => colonyNetwork.deposit(new BigNumber('1000000000000000000'), { from: addr })));
    ...
    it('should prevent a hash from advancing if it might still get an opponent', async () => {
      ...
      for (let i = 0; i < 8; i += 1) {
        await giveUserCLNYTokens(accountsForTest[i], new BigNumber('1000000000000000000')); // eslint-disable-line no-await-in-loop
        // These have to be done sequentially because this function uses the total number of tasks as a proxy for getting the
        // right taskId, so if they're all created at once it messes up.
      }
      await Promise.all(accountsForTest.map(addr => clny.approve(colonyNetwork.address, new BigNumber('1000000000000000000'), { from: addr })));
      await Promise.all(accountsForTest.map(addr => colonyNetwork.deposit(new BigNumber('1000000000000000000'), { from: addr })));
    ...
    it('should not allow a hash to be invalidated multiple times, which would move extra copies of its opponent to the next stage', async () => {
      ...
      await repCycle.submitNewHash('0x87654321', 10, 10, { from: OTHER_ACCOUNT });
      await accommodateChallengeAndInvalidateHash(this, 0, 1);
      testHelper.checkErrorRevert(accommodateChallengeAndInvalidateHash(this, 0, 1));
    });
  });
});
